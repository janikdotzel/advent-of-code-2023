def part1(values: list[str]) -> int:
    # remove all non-numeric characters from the string, but keep the amount of items
    reduced_values: list[str] = list(map(lambda x: ''.join(filter(str.isdigit, x)), values.splitlines()))
    print(reduced_values)

    # only keep the first and last number of each item
    first_and_last_numbers: list[str] = list(map(lambda x: ''.join([x[0], x[-1]]), reduced_values))
    print(first_and_last_numbers)

    # convert all strings to integers
    int_numbers: list[int] = list(map(lambda x: int(x), first_and_last_numbers))
    print(int_numbers)
