...
def part2(values: list[str]) -> int:
    ...
    # Check if a value contains a key from the dictionary. If it does, replace the key with the value. After that, search for the next key in the current value.
    # If the value does not contain a key, add the value to the total.
    for value in values:
        for key in dictionary.keys():
            while key in value:
                value: list[str] = value.replace(key, str(dictionary[key]), 1)

        # remove any non numeric character from value
        value: list[str] = ''.join(filter(lambda x: x.isdigit(), value))
        new_values.append(value)
    print(new_values)

    # only keep the first and last number of the value
    new_values: list[str] = list(map(lambda x: ''.join([x[0], x[-1]]), new_values))
    print(new_values)

    # convert the list of strings to a list of ints
    new_values: list[int] = list(map(int, new_values))
