...
def part1(games: list[Game]) -> int:
    available_cubes: dict[str, int] = {
        "red": 12,
        "green": 13,
        "blue": 14
    }
    possible_games: dict[Game, bool] = []

    for game in games:
        possible = True
        for cube_set in game.cube_sets:
            temp_available_cubes = available_cubes.copy()
            for cube in cube_set.cubes:
                temp_available_cubes[cube.color] -= cube.number
        
            if all([temp_available_cubes[color] >= 0 for color in temp_available_cubes]):
                possible = True
            else:
                possible = False
                break
    ...
    print([game.id for game in possible_games])
    return sum([game.id for game in possible_games])

def convert_string_to_games(string: str) -> list[Game]:
    game_strings = string.split("\n")
    games = []

    for game_string in game_strings:
        game_parts = game_string.split(": ")
        game_id = int(game_parts[0].split(" ")[1])
        cube_sets_str = game_parts[1].split("; ")
        cube_sets = []

        for cube_set_str in cube_sets_str:
            cubes_str = cube_set_str.split(", ")
            cubes = []

            for cube_str in cubes_str:
                cube_parts = cube_str.split(" ")
                cube = Cube(cube_parts[1], int(cube_parts[0]))
                cubes.append(cube)
...
games: list[Game] = convert_string_to_games(input)
